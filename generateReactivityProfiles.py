...
def GetConfigparameters():
    ...
    conf.read("Shape.cfg")
    global  outputdir,minDepth,normProfile,maxBackground,minNormDepth,filterStderr,normProfile,ignoreDeletions,filterStderr
    outputdir=os.path.join( os.getcwd(),conf.get("Paths","outputdir"))
    minDepth=int(conf.get("ProfilesgenerationParameters","minDepth"))
    normProfile=conf.get("ProfilesgenerationParameters","normProfile")
    maxBackground=float(conf.get("ProfilesgenerationParameters","maxBackground"))
    minNormDepth=int(conf.get("ProfilesgenerationParameters","minNormDepth"))
    filterStderr= conf.get("ProfilesgenerationParameters","filterStderr")
    normProfile= conf.get("ProfilesgenerationParameters","normProfile")
    ignoreDeletions=conf.get("ProfilesgenerationParameters","ignoreDeletions")
    filterStderr=conf.get("ProfilesgenerationParameters","filterStderr")
...
def combineData(csvFile):
    ...
    seq = {}
    combine=True
    fileIn = open(csvFile, "rU")
    print "loading data from file %s"%csvFile
    sys.stdout.flush()
    for line in fileIn:
	...
	if(combine==True):
        	if len(splitLine) > 5:
                    ...
                    #print lineTargetName
                    lineType = splitLine[2]
                    if lineTargetName == lineTargetName :
                        ...
                        if "sequence" in lineType:
                            ...
                            for item in dataList:
                                if item!='':
                                    ...
                                    break
                        elif "->" in lineType or (ignoreDeletions==False and "del" in lineType and "deletion" not in lineType):
                            i = 0
                            for item in dataList:
                                if item!='':
...
def writeFigures(num,reactivity,stdev,
    ...
    leg = ax2.legend(["SHAPE","Untreated"], loc=2, borderpad=0.8, handletextpad=0.2)
    ...
    ax3.legend(["SHAPE ","Untreated"], loc=2, borderpad=0.8, handletextpad=0.2)
...
def writeHistograms(num,normShape,normStderr,
    ...
    ax1.set_title('Mutation rates', x=0.5,y=4) 
    #y = title.get_y()
    #title.set_y(y
    ...
    leg = ax1.legend(["SHAPE","Untreated","Denatured"], fontsize=11)
    ...
    drawPercentile(ax1,filteredDcRate,95.0, "black", x1=0,x2=1, y=-1.8, percentage=True, name="Denatured sample:\n 95th percentile rate:")
    ...
    drawPercentile(ax2,dcDepth,50.0, "black", x1=0,x2=1, y=-1.8, name="Denatured sample:\n Median depth:")
    ...
    leg = ax3.legend(["Reactivities","Stderrs"], fontsize=11)
...
try:
    ...
    rxRates,rxDepth,seq = combineData( rxNames)

    # make combined minus reagent mutation freq list
    bgRates, bgDepth, unused = combineData(bgNames)

    # make combined denatured control mutation freq list
    dcRates, dcDepth, unused = combineData(dcNames)
    ...
    filteredProfile, filteredStderr = filterProfile(profile, stderr, bgRates, rxDepth, bgDepth, dcDepth,
                                    maxBackground=maxBackground, minDepth=minDepth)
    ...
    outputDir="./output/"
    # write .SHAPE file
    fileOutPath = os.path.join(outputDir,"reactivity_profiles/"+profileName+".shape")
    ...
    # write .tab file
    fileOutPath = os.path.join(outputDir,"reactivity_profiles/"+profileName+".tab")
    fileOut = open(fileOutPath, "w")
    ...
    # write .map file
    fileOutPath = os.path.join(outputDir, "reactivity_profiles/"+profileName+".map")
    fileOut = open(fileOutPath, "w")
    nuc = 1
    for i in range(len(normProfile)):
